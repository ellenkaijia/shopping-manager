...
var nationalities=new Array('中国大陆' , '香港', '澳门' ,'台湾','澳大利亚(Australia)','加拿大(Canada)','法国(France)','德国(Germany)','印度尼西亚(Indonesia)','意大利(Italy)','日本(Japan)','韩国(Korea)','马来西亚(Malaysia)','新西兰(New Zealand)','荷兰(Netherlands)','菲律宾(Philippines)','俄罗斯(Russia)','新加波(Singapore)','瑞典(Sweden)','泰国(Thailand)','英国(UK)','乌克兰(Ukraine)','美国(USA)','越南(Vietnam)');
//国籍初始化
//2014-05-21  by HongChuan
var nationalityInit = function(nationalityId, defaultNationality)
{
	var v_nationalityId = document.getElementById(nationalityId);
	for(var i=0; i<nationalities.length; i++) 
	{
		var option = document.createElement("OPTION");
		option.innerHTML = nationalities[i];
		option.value = nationalities[i];
		v_nationalityId.options.add(option);
		$("#"+nationalityId).val(defaultNationality);
	}
};
